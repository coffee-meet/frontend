    ...
    <GradationBackground isDarkMode={isDarkMode}>
      ...
      <PageContainer isDarkMode={isDarkMode} height={'80%'}>
        <FlexBox direction={'column'}>
          ...
          <RegisterInput width={343} placeholder={'회사 이름'} ref={nameRef} />
          <Spacing size={20} />
          <SectionLabelText width={390}>{'이메일'}</SectionLabelText>
          <FlexBox gap={10}>
            <RegisterInput width={260} placeholder={'회사 이메일'} ref={emailRef} />
            <NormalButton normalButtonType={'email-certify'} onClick={handleVerifyEmail}>
              {'이메일 인증'}
            </NormalButton>
          </FlexBox>
          <Spacing size={16} />
          ...
          >
            <RegisterInput width={343} placeholder={'인증코드 6자리 입력'} ref={codeRef} />
          ...
          <label htmlFor={'card-image-upload'}>
            ...
            {imageSrc ? (
              <StyleImageCard src={imageSrc} alt={'명함 이미지'} />
            ) : (
              <CardImageFallback>
