...
const RegisterCompany = () => {
  ...
  const submitUserCompanyData = () => {
    ...
    const body = {
      companyEmail: emailRef.current && emailRef.current.value,
      jobList: interestList,
    }
    registerCompanyMutation.mutate(body)
  }

  const registerCompanyData = async (body: object) => {
    return await axiosAPI.post('/v1/certification/users/me/company-info', body)
  }
  ...
  return (
    <StyleRegisterWrapper>
      ...
      <FlexBox gap={16}>
        ...
        <NormalButton normalButtonType={'email-certify'} onClick={() => handleEmailCertification()}>
          {'이메일 인증'}
        </NormalButton>
      </FlexBox>
      {alertInfo ? (
